    ...
    def _generate_architecture_section(self) -> str:
        ...
        if "django-urls" in self.results:
            ...
            if "summary" in urls:
                ...
                # Collect all URL patterns with their views
                if "results" in urls:
                    path_count = 0
                    include_count = 0
                    all_patterns = []
                    ...
                    for file_path, file_patterns in urls["results"].items():
                        if isinstance(file_patterns, dict) and "urlpatterns" in file_patterns:
                            for pattern in file_patterns["urlpatterns"]:
                                ...
                                # Collect pattern info for the table
                                if pattern_type == "path":
                                    url_pattern = pattern.get("pattern", "")
                                    view_module = pattern.get("view_module") or ""
                                    view_name = pattern.get("view_name") or ""
                                    
                                    # Create full view path
                                    if view_module and view_name:
                                        full_view = f"{view_module}.{view_name}"
                                    elif view_name:
                                        full_view = view_name
                                    elif view_module:
                                        full_view = view_module
                                    else:
                                        full_view = "N/A"
                    ...
                    lines.append(f"\n- **Include Patterns**: {include_count}")

                    # Show complete URL to view mapping
                    if all_patterns:
                        lines.append("\n#### Complete URL to View Mapping")
                        lines.append(f"\nShowing all {len(all_patterns)} URL patterns with their corresponding view implementations:")
                        lines.append("\n| URL Pattern | View Implementation | Name |")
                        lines.append("|-------------|---------------------|------|")
                        
                        # Sort by URL pattern for better readability
                        all_patterns.sort(key=lambda x: x["pattern"] or "")
                        
                        for p in all_patterns:
                            # Truncate long patterns and views
                            ...
                            lines.append(f"| `{pattern}` | `{view}` | {name} |")
                        
                        # Also show top files with most patterns for summary
                        patterns_by_file = {}
                        for file_path, file_patterns in urls["results"].items():
                            if isinstance(file_patterns, dict) and "urlpatterns" in file_patterns:
