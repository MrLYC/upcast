...
def _add_direct_references(index: RouteIndex, context: RouteModule, bindings: dict[str, str]) -> None:
    for call_node in _module_route_calls(context.module, bindings):
...
def _module_route_calls(module: nodes.Module, bindings: dict[str, str]) -> list[nodes.Call]:
    """Return direct route calls owned by module-level ``urlpatterns`` code."""
    calls: list[nodes.Call] = []
    for statement in module.body:
        values: list[nodes.NodeNG] = []
        if isinstance(statement, nodes.Assign) and any(
            isinstance(target, nodes.AssignName) and target.name == "urlpatterns" for target in statement.targets
        ):
            values.append(statement.value)
        elif isinstance(statement, nodes.AugAssign) and isinstance(statement.target, nodes.AssignName):
            if statement.target.name == "urlpatterns":
                values.append(statement.value)
        elif isinstance(statement, nodes.Expr) and isinstance(statement.value, nodes.Call):
        ...
        for value in values:
            calls.extend(
                call
                for call in value.nodes_of_class(nodes.Call)
                if _is_route_call(call, bindings) and len(call.args) >= 2
            )
    return calls
