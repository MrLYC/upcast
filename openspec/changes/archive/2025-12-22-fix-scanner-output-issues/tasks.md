...
- [x] Update `parse_model()` signature in `upcast/common/django/model_parser.py`
...
- [x] Add `_calculate_module_path(file_path: Path, root_path: Path) -> str` helper function
...
- [x] Replace qname-based module extraction with file-based calculation
...
- [x] Pass file context to parser in `_scan_file()`
...
- [x] Test `_calculate_module_path()` function
...
- [x] Update existing Django model scanner tests
...
- [x] Test scan-django-models on wagtail
...
- [x] Create helper methods in `HttpRequestsScanner`:
...
- [x] Add `_normalize_url_pattern(node: nodes.NodeNG) -> str | None` method
...
- [x] Test updated `_extract_url()` flow:
...
- [x] Test scanner with realistic HTTP code:
...
- [x] Test scan-http-requests on wagtail/embeds
...
- [x] Update README.md scanner examples
...
- [x] Document breaking changes (improvements):
...
- [x] Run both scanners on user's test case (wagtail):
...
- [x] Verify ruff compliance: `uv run ruff check`
- [x] Verify coverage maintained: `uv run pytest --cov`
