...
### YAML Normalization Strategy

The comparison process will:

1. Extract the `results` section from YAML (ignore metadata like timestamps)
2. Parse YAML to native data structures
3. Sort dictionary keys recursively
4. For specific metadata arrays (like `blocks`, `attributes`), sort to ensure stability
5. Re-serialize to YAML with consistent formatting (single style for quotes, sorted keys)
6. Perform text diff on normalized output

This preserves meaningful semantic changes while eliminating formatting noise.

### PyPI Publishing Security

- Use repository secrets for PYPI_TOKEN
- Publishing workflow requires write permissions to contents (for checkout) but not for other resources
- Tag must be annotated or lightweight (both supported)
- Version validation prevents accidental publishes with wrong version numbers
