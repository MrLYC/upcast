...
2. Update `.pre-commit-config.yaml` rev to match (or keep v0.5.2 if compatible)
...
3. Identify missing scanners (especially cyclomatic-complexity)
   **Validation**:
...
1. Add section under "Code Quality Scanners" (after Architecture)
2. Include command syntax, options, output example
3. Document severity levels and thresholds
4. Add to Quick Start examples if appropriate
   **Validation**:
...
1. Count total scanners from main.py
2. Update "12 specialized scanners" if count changed
3. Update any other references to scanner count
   **Validation**:
...
3. Fix any broken commands or syntax
   **Validation**:
