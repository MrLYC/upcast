...
- [ ] **Create module structure**: Create `upcast/prometheus_metrics_scanner/` directory with `__init__.py`, `cli.py`, `checker.py`, `metrics_parser.py`, `ast_utils.py`, `export.py`

  - Establishes basic module layout following project conventions
  - Defines public API exports in `__init__.py`

- [ ] **Implement AST pattern detection utilities**: Create `ast_utils.py` with functions:
...
- [ ] **Create test structure**: Set up `tests/test_prometheus_metrics_scanner/` with:
...
- [ ] **Test AST utilities**: Write unit tests for all `ast_utils.py` functions
...
- [ ] **Implement metric instantiation parser**: Create `metrics_parser.py` with:
...
- [ ] **Add MetricInfo and UsageInfo dataclasses**: Define data structures in `metrics_parser.py`:

  ```python
  @dataclass
  class UsageInfo:
      location: str
      pattern: str  # instantiation, decorator, custom_collector, etc.
      statement: str
...
- [ ] **Create parser test fixtures**: Add `fixtures/decorators.py` and `fixtures/custom_collectors.py`
...
- [ ] **Test metric instantiation parsing**: Write tests in `test_metrics_parser.py`
...
- [ ] **Implement decorator pattern detection**: Add to `metrics_parser.py`:
...
- [ ] **Implement custom collector parsing**: Add to `metrics_parser.py`:
...
- [ ] **Test decorator detection**: Extend `test_metrics_parser.py`
...
- [ ] **Test custom collector parsing**: Extend `test_metrics_parser.py`
...
- [ ] **Implement PrometheusMetricsChecker**: Create `checker.py` with:
...
- [ ] **Implement metric aggregation logic**: Add to `checker.py`:
...
- [ ] **Create complex test fixture**: Add `fixtures/complex.py`
...
- [ ] **Add file filtering**: Implement in `cli.py`:
...
- [ ] **Update project dependencies**: Modify `pyproject.toml`
...
- [ ] **Validate with ruff**: Ensure code quality
...
3. Check test coverage: `uv run pytest --cov=upcast.prometheus_metrics_scanner --cov-report=term-missing`
4. Run `openspec validate implement-prometheus-metrics-scanner --strict`
